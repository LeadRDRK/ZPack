...
void Reader::UnpackFiles(EntryList &fileList, BufferList &bufferList)
{
    for (int i = 0; i < fileList.size(); ++i) {
        FileInfo *info = fileList[i];
        auto dstPair = bufferList[i];
        char* dst = dstPair.first;
        int dstCapacity = dstPair.second;
        UnpackFile(info, dst, dstCapacity);
    }
}

void Reader::UnpackFiles(EntryList &fileList, OStreamList &streamList)
{
    for (int i = 0; i < fileList.size(); ++i) {
        FileInfo *info = fileList[i];
        UnpackFile(info, *streamList[i]);
    }
}

void Reader::UnpackFiles(FileNameList &fileList, BufferList &bufferList)
{
    for (int i = 0; i < fileList.size(); ++i) {
        FileInfo *info = GetFileInfo(fileList[i]);
        auto dstPair = bufferList[i];
        char* dst = dstPair.first;
        int dstCapacity = dstPair.second;
        UnpackFile(info, dst, dstCapacity);
...
void Reader::UnpackFiles(FileNameList &fileList, OStreamList &streamList)
{
    for (int i = 0; i < fileList.size(); ++i) {
        FileInfo *info = GetFileInfo(fileList[i]);
        UnpackFile(info, *streamList[i]);
